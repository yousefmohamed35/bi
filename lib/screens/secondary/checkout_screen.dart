...
extension _CheckoutScreenL10n on AppLocalizations {
  String get cashRequestPaymentName =>
      localeName == 'ar' ? 'طلب شراء نقدي' : 'Cash Request';

  String get cashRequestPaymentDescription => localeName == 'ar'
      ? 'إرسال طلب شراء نقدي ليتم مراجعته من الإدارة'
      : 'Send a cash purchase request for admin review';

  String get cashRequestSuccessMessage => localeName == 'ar'
      ? 'تم إرسال طلب الشراء النقدي بنجاح. بانتظار مراجعة الإدارة.'
      : 'Cash purchase request sent successfully. Waiting for admin review.';

  String get cashRequestAdminNoteHint => localeName == 'ar'
      ? 'ملاحظة للإدارة (اختياري) - مثال: أريد الدفع كاش في الفرع'
      : 'Note for admin (optional) - e.g. I want to pay cash at the branch';
}
...
class _CheckoutScreenState extends State<CheckoutScreen> {
  ...
  List<Map<String, dynamic>> _getPaymentMethods(BuildContext context) {
    ...
    return [
      {
        'id': 'cash',
        'name': l10n.cashRequestPaymentName,
        'icon': Icons.account_balance,
        'description': l10n.cashRequestPaymentDescription,
      },
      {
        'id': 'wallet',
        'name': l10n.eWallet,
  ...
  Future<void> _handleCheckout() async {
    ...
    try {
      // Cash request flow for student purchase requests
      if (_selectedPayment == 'cash') {
        await PaymentsService.instance.createCashCourseRequest(
          courseId: courseId,
          message: _cashRequestMessage,
        );
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                AppLocalizations.of(context)!.cashRequestSuccessMessage,
                style: GoogleFonts.cairo(),
              ),
              backgroundColor: const Color(0xFF10B981),
              behavior: SnackBarBehavior.floating,
                          ...
                          children: [
                            ...
                            if (_selectedPayment == 'cash') ...[
                              ...
                              TextField(
                                maxLines: 3,
                                decoration: InputDecoration(
                                  hintText: l10n.cashRequestAdminNoteHint,
